-- ESP Hack Simple for Roblox (with NameTags)
...
mainFrame.Size = UDim2.new(0, 220, 0, 160) -- a bit taller to fit nametags
...
-- Create ESP box & name tag for a player
...
RunService.RenderStepped:Connect(function()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            nameTags[player].Adornee = rootPart
        else
            removeEsp(player)
        end
    end
